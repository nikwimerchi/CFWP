	...
	*/
	plotDataLabelsText(opts) {
		...
		if (correctedLabels.textRects) {
			if (x < -20 - correctedLabels.textRects.width || x > w.layout.gridWidth + correctedLabels.textRects.width + 30) text = "";
		}
	...
	*/
	...
	createLegendMarker({ i, fillcolor }) {
		...
		if (w.config.legend.markers.customHTML) {
			...
			const SVGMarker = (Environment.isBrowser() ? window.SVG : global.SVG)().addTo(elMarker).size("100%", "100%");
...
var Helpers$2 = class Helpers3 {
	...
	*/
	getStringX(x) {
		...
		const w = this.w;
		let rX = x;
		if (w.config.xaxis.convertedCatToNumeric && w.labelData.categoryLabels.length) x = w.labelData.categoryLabels.indexOf(String(x)) + 1;
	...
	*/
	getFontSize(coordinates) {
		...
		function fontSize(width, height) {
			...
			const arearoot = Math.pow(area, .5);
